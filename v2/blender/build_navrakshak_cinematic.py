...
def wake_curve(name, y, material):
    ...
    obj=bpy.data.objects.new(name,cu_data); bpy.context.collection.objects.link(obj)
    cu_data.materials.append(material); return obj
...
boom=bpy.context.object; boom.name='Radar Boom'; boom.data.materials.append(dark); boom.parent=root
...
# wake
for y in (-.8,.8):
    w=wake_curve('Wake',y,cyan); w.parent=root

# geofence volume
bpy.ops.mesh.primitive_cylinder_add(vertices=96,radius=18,depth=4,location=(0,0,1))
gf=bpy.context.object; gf.name='3D GEOFENCE'; gf.display_type='WIRE'; gf.data.materials.append(cyan)
...
cam=bpy.context.object; cam.name='Cinematic Camera'; bpy.context.scene.camera=cam
def look(obj,pt): obj.rotation_euler=(Vector(pt)-obj.location).to_track_quat('-Z','Y').to_euler()
look(cam,(0,0,1.5)); cam.keyframe_insert('location',frame=1); cam.keyframe_insert('rotation_euler',frame=1)
cam.location=(18,28,11); look(cam,(3,0,1.3)); cam.keyframe_insert('location',frame=180); cam.keyframe_insert('rotation_euler',frame=180)
...
sc=bpy.context.scene; sc.frame_start=1; sc.frame_end=240; sc.render.engine='BLENDER_EEVEE_NEXT'
